...
class _IncomingTabState extends State<_IncomingTab> {
  ...
  Future<void> _respond(ConnectionRequestItem item, bool accept) async {
    try {
      if (accept) {
        if (!mounted) return;
        final inverse = await showDialog<String>(
          context: context,
          builder: (dialogContext) => SimpleDialog(
            title: const Text('请选择你与对方的关系'),
            children: [
              Padding(
                padding: const EdgeInsets.fromLTRB(24, 0, 24, 8),
                child: Text('对方说"我是你的${_relationLabel(item.relation)}"，你是对方的：',
                    style: const TextStyle(fontSize: 12, color: Colors.grey)),
              ),
              ...relationOptions.map((option) => SimpleDialogOption(
                    onPressed: () => Navigator.pop(dialogContext, option.code),
                    child: Text('我是对方的${option.label}'),
                  )),
            ],
          ),
        );
